...
describe('maybeSetAuthorizationTokensForRegistries', () => {
  it.each([
    ...
    expect(configuration.values.get('npmRegistries').get(awsCodeArtifactRegistry2).get(NPM_ALWAYS_AUTH)).toBe(true)
    expect(configuration.values.get('npmRegistries').get(awsCodeArtifactRegistry2.slice(6)).get(NPM_AUTH_TOKEN)).toBe(
      'test-token-2-null'
    )
    expect(configuration.values.get('npmRegistries').get(awsCodeArtifactRegistry2.slice(6)).get(NPM_ALWAYS_AUTH)).toBe(
      true
    )
    expect(
      configuration.values.get('npmRegistries').get(awsCodeArtifactRegistry2.slice(0, -1)).get(NPM_AUTH_TOKEN)
    ).toBe('test-token-2-null')
    expect(
      configuration.values.get('npmRegistries').get(awsCodeArtifactRegistry2.slice(0, -1)).get(NPM_ALWAYS_AUTH)
    ).toBe(true)
    expect(configuration.values.get('npmRegistries').get('//x.com').get(NPM_AUTH_TOKEN)).toBe(undefined)
    expect(configuration.values.get('npmRegistries').get('//x.com').get(NPM_ALWAYS_AUTH)).toBe(undefined)
    expect(configuration.values.get('npmRegistries').get(awsCodeArtifactRegistry4).get(NPM_AUTH_TOKEN)).toBe(
      'already-here'
    )
    ...
    expect(configuration.values.get('npmScopes').get('scope-b').get(NPM_ALWAYS_AUTH)).toBe(undefined)
    expect(configuration.values.get('npmScopes').get('scope-c').get(NPM_AUTH_TOKEN)).toBe('already-here')
