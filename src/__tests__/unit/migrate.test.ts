...
describe('computeMigratedLocator', () => {
  ...
  ])('should determine the registry type based on command-line arguments: %p', (resolution, registry, expected) => {
    const newLocator = computeMigratedLocator(resolution, registry)
    const newLocatorString = newLocator !== null ? structUtils.stringifyLocator(newLocator) : null
