...
import { Configuration, StreamReport, CommandContext, Locator, structUtils } from '@yarnpkg/core'
 ...
 */
export const migrateLockFile = async (configuration: Configuration, report: StreamReport): Promise<void> => {
  const lockfilePath = ppath.join(configuration.startingCwd, configuration.get('lockfileFilename'))
  const content = await xfs.readFilePromise(lockfilePath, 'utf8')
  const parsed = parseSyml(content)

  const lockfileEntries: [string, LockFileEntry][] = Object.entries(parsed).filter(([key]) => !key.startsWith('__'))
  let migratedCount = 0

  for (const [key, { resolution }] of lockfileEntries) {
    const resolvedLockfileEntry = resolveLockfileEntry(configuration, resolution)
    ...
    report.reportInfo(null, `Migrated ${oldLocatorString} => ${newLocatorString}`)
 ...
 */
export const computeMigratedLocator = (resolution: string, registry: string): Locator | null => {
  const oldLocator = structUtils.parseLocator(resolution)
  ...
  const archiveUrlObj = URL.parse(
    `${registry}/${structUtils.stringifyIdent(oldLocator)}/-/${oldLocator.name}-${version}.tgz`
  )
